    ...
    for (let name in global.plugins) {
      ...
      if ((usedPrefix = (match[0] || "")[0])) {
        ...
        try {
          ...
          if (m.diamond) m.reply(`*${+m.diamond}* Diamante usado`);
        }
        break;
      }
    }
                ...
                ? (
                    ...
                    .replace(
                      ...
                      groupMetadata.desc?.toString() ||
                        "*⚠️ ESTE GRUPO NO TIENE DESCRIPCIÓN ⚠️*"
                    )
    ...
    if (cs.isGroup == false) {
      if (cs.status == "offer") {
        let callmsg = await this.reply(
          nk.from,
          `Hola *@${cs.from.split("@")[0]}*, Las ${
            cs.isVideo ? "videollamadas" : "llamadas"
          } están prohibidas, seras bloqueado`,
          false,
          { mentions: [cs.from] }
        );
        let vcard = `BEGIN:VCARD\nVERSION:3.0\nN:;JuliusSauco 👑;;;\nFN:JuliusSauco\nORG:JuliusSauco 👑\nTITLE:\nitem1.TEL;waid=18134039996:+598 9999\nitem1.X-ABLabel:JuliusSauco 👑\nX-WA-BIZ-DESCRIPTION:Solo cosas del bot.\nX-WA-BIZ-NAME:JuliusSauco 👑\nEND:VCARD`;
        await this.sendMessage(
          cs.from,
          {
            contacts: { displayName: "JuliusSauco 👑", contacts: [{ vcard }] },
          },
          { quoted: callmsg }
        );
...
global.dfail = (type, m, conn) => {
  ...
  let msg = {
    ...
    unreg:
      "⚠️ *REGÍSTRESE PARA USAR ESTA FUNCIÓN ESCRIBIENDO:*\n\n• */reg nombre.edad*\n\n*_❕ Ejemplo_* : */reg JuliusSauco.25*",
