...
let handler = async (
  m,
  { conn, usedPrefix, usedPrefix: _p, __dirname, text, command }
) => {
  try {
    let _package =
      JSON.parse(
    ...
    await conn.reply(m.chat, "*Enviando el menu . . .*", fload, {
      contextInfo: {
...
〬╚════ ≪ •❈• ≫ ════╝`;
    ...
    conn.reply(
      m.chat,
      "*⚠️ EL MENU TIENE UN ERROR PRUEBE CON EL MENU COMPLETO: (.menucompleto)*",
      fpoll,
      m
    );
  }
