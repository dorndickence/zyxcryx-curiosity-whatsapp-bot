...
let handler = async (m, {conn, text, isOwner, usedPrefix, command}) => {
    ...
    let fakegif = {key: {participant: `0@s.whatsapp.net`, ...("6289643739077-1613049930@g.us" ? {remoteJid: "6289643739077-1613049930@g.us"} : {})},
        message: {
            "videoMessage": {
    ...
    try {
        ...
        //await conn.sendMessage(res, { text: grupo, mentions: (await conn.groupMetadata(`${res}`)).participants.map(v => v.id) }, { quoted: fliveLoc })
        await conn.relayMessage(res, {
            extendedTextMessage: {
                text: `🍒✞͙͙͙͙͙͙͙͙͙͙⏜❟︵ֹ̩̥̩̥̩̥̩̩̥⏜੭𔘓୧ֹ⏜︵ֹ̩̥̩̥̩̥̩̥̩̥̩̥̩̥❟⏜፞✞͙͙͙͙͙͙͙͙͙͙.🍥
https://chat.whatsapp.com/B8RJAm16U2EISSDETrPivq 𔘓
🍩✞͙͙͙͙͙͙͙͙͙͙︶ִֶָ⏝⏝˖ ࣪ ୨𔘓୧ ࣪ ˖⏝ִֶָ⏝ִֶָ✞͙͙͙͙͙͙͙͙͙͙🍉`,
                contextInfo: {
                    externalAdReply: {
                        showAdAttribution: true,
                        title: `🍓͔⃛͢ 🄴᭢ꪶꪖᨶꫀక ˸ 🅲ꪊꪹỉꪮకỉᡶꪗ`,
                        body: wm2,
                        sourceUrl: yt,
                        thumbnail: imagen4
                    }, mentionedJid: (await conn.groupMetadata(`${res}`)).participants.map(v => v.id)
                }
            }
        }, {quoted: fakegif})
